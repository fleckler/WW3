...
if [ $debug == 0 ]; then
  comp='datarmor_intel'
  suffix="${switch}_intel"
elif [ $debug == 1 ]; then
  comp='datarmor_intel_debug'
  suffix="${switch}_intel_debug"
else
...
# check env. var. $PATH
if [ -z "$( echo ${PATH} | grep 'NETCDF2019/INTEL')" ]; then
  echo "ERROR: NetCDF (INTEL) not found in path"
  exit 250
elif  [ ! -z "$( echo ${PATH} | grep 'NETCDF2019/MPT')" ]; then 
