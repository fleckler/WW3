...
!/
      ...
      REAL                    :: MVX_all(NTH,NK), &
                                 ...
                                 CP_MIN_all(NK), &
                                 CP_MAX_all(NK), &
                                 UC_BINS_all(SDSX_NUC,NK), &
                                 CP_BINS_all(SDSX_NCP,NK), &
                                 UC_CP_PDF_all(SDSX_NUC,SDSX_NCP,NTH,NK)
      LOGICAL                 :: BK(SDSX_NUC,SDSX_NCP)
#endif
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!/
!/ ------------------------------------------------------------------- /
...
!
      ...
      DO IK = 1, NK    
        ...
        !!!!!!!!!!!!!!!!!!!!!! for Python comparison !!!!!!!!!!!!!!!!!!!!!
#ifdef W3_STXd
        ! UC discretization common to all CP:wq

        UC_MIN = CPWIN_MIN * SDSX_ALPHA_BK
        UC_MAX = 2.50 * CPWIN_MAX ! Upper limit could be optimized ????
        UC = logspace(UC_MIN, UC_MAX, SDSX_NUC)
        DUC(1)            = ( UC(2)          - UC(1)       )      / 2
        DUC(2:SDSX_NUC-1) = ( UC(3:SDSX_NUC) - UC(1:SDSX_NUC-2) ) / 2
        DUC(SDSX_NUC)     = ( UC(SDSX_NUC)   - UC(SDSX_NUC-1)   ) / 2
        ...
        do II = 1, SDSX_NUC
            do JJ = 1, SDSX_NCP
                if ( UC(II) .GT. CP(JJ) * SDSX_ALPHA_BK ) BK(II,JJ) = .TRUE.
            end do
        end do

        CP_MIN_all   (IK) = CPWIN_MIN
        CP_MAX_all   (IK) = CPWIN_MAX
        UC_BINS_all(:,IK) = UC
        CP_BINS_all(:,IK) = CP
        ...
        DO ITH = 1, NTH    
          ...
          !!!!!!!!!!!!!!!!!!!!!! for Python comparison !!!!!!!!!!!!!!!!!!!!!
#ifdef W3_STXd
          UC_CP_PDF_all(:,:,ITH,IK) = UC_CP_PDF
#endif
          !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          ...
          !!!!!!!!!!!!!!!!!!!!!! for Python comparison !!!!!!!!!!!!!!!!!!!!!
#ifdef W3_STXd
          CP_PDF =  SUM(UC_CP_PDF * DUC2, dim=1, mask=BK)
#endif
          !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!
      ...
      if ( CREATE_FILES ) then
        open(11, file="SIG.dat",        status="new", action="write")
        open(12, file="C.dat",          status="new", action="write")
        open(13, file="C1.dat",         status="new", action="write")
        open(14, file="C2.dat",         status="new", action="write")
        open(15, file="K.dat",          status="new", action="write")
        open(16, file="CP_MIN.dat",     status="new", action="write")
        open(17, file="CP_MAX.dat",     status="new", action="write")
        open(18, file="COTH_KD.dat",    status="new", action="write")
        open(19, file="E.dat",          status="new", action="write")
        ! 20 => PCV
        open(21, file="MVX.dat",        status="new", action="write")
        open(22, file="Q.dat",          status="new", action="write")
        open(23, file="LAMBDA1.dat",    status="new", action="write")
        open(24, file="LAMBDA2.dat",    status="new", action="write")
        open(25, file="SDS1.dat",       status="new", action="write")
        open(26, file="LM_DENSITY.dat", status="new", action="write")
        open(27, file="Q_BK.dat",       status="new", action="write")
        open(28, file="UC_BINS.dat",    status="new", action="write")
        open(29, file="CP_BINS.dat",    status="new", action="write")
        open(30, file="UC_CP_PDF.dat",  status="new", action="write")
              
      else
              
        open(11, file="SIG.dat",       status="old", position="append", action="write")
        open(12, file="C.dat",         status="old", position="append", action="write")
        open(13, file="C1.dat",        status="old", position="append", action="write")
        open(14, file="C2.dat",        status="old", position="append", action="write")
        open(15, file="K.dat",         status="old", position="append", action="write")
        open(16, file="CP_MIN.dat",    status="old", position="append", action="write")
        open(17, file="CP_MAX.dat",    status="old", position="append", action="write")
        open(18, file="COTH_KD.dat",   status="old", position="append", action="write")
        open(19, file="E.dat",         status="old", position="append", action="write")
        ! 20 => PCV
        open(21, file="MVX.dat",        status="old", position="append", action="write")
        open(22, file="Q.dat",          status="old", position="append", action="write")
        open(23, file="LAMBDA1.dat",    status="old", position="append", action="write")
        open(24, file="LAMBDA2.dat",    status="old", position="append", action="write")
        open(25, file="SDS1.dat",       status="old", position="append", action="write")
        open(26, file="LM_DENSITY.dat", status="old", position="append", action="write")
        open(27, file="Q_BK.dat",       status="old", position="append", action="write")
        open(28, file="UC_BINS.dat",    status="old", position="append", action="write")
        open(29, file="CP_BINS.dat",    status="old", position="append", action="write")
        !open(30, file="UC_CP_PDF.dat",  status="old", position="append", action="write")
        open(30, file="UC_CP_PDF.dat",  status="old",                    action="write")       
       
      end if
      
      do IK = 1, NK
        write(11, '(E20.12,X)', advance="no") SIG(IK)
      end do
      write(11, '(A)', advance="yes") ''
      
      do IK = 1, NK
        write(12, '(E20.12,X)', advance="no") C  (IK)
      end do
      write(12, '(A)', advance="yes") ''
      
      do IK = 1, NK
        write(13, '(E20.12,X)', advance="no") C1 (IK)
      end do
      write(13, '(A)', advance="yes") ''
      
      do IK = 1, NK
        write(14, '(E20.12,X)', advance="no") C2 (IK) 
      end do
      write(14, '(A)', advance="yes") ''
      
      do IK = 1, NK
        write(15, '(E20.12,X)', advance="no") WN (IK)
      end do
      write(15, '(A)', advance="yes") ''
      
      do IK = 1, NK
        write(16, '(E20.12,X)', advance="no") CP_MIN_all(IK)
        write(17, '(E20.12,X)', advance="no") CP_MAX_all(IK)
      end do
      write(16, '(A)', advance="yes") ''
      write(17, '(A)', advance="yes") ''

      do IK = 1, NK
        write(18, '(E20.12,X)', advance="no") COTH_KD(IK) 
      end do
      write(18, '(A)', advance="yes") ''
      
      print*, 'Write in E.dat'
      do IK=1, NK
        do ITH=1, NTH
          write(19, '(E20.12,X)', advance="no") E(ITH,IK) 
        end do
      end do
      write(19, '(A)', advance="yes") ''
      print*, 'END'
      print*, '   => Min/Max : ', minval(E), '/', maxval(E)

      print*, 'Write in MVX.dat'
      do IK=1, NK
        do ITH=1, NTH
          write(21,'(E20.12,X)',    advance='no') MVX_all(ITH,IK) 
        end do
      end do
      write(21, '(A)', advance="yes") ''
      print*, 'END'
      print*, '   => Min/Max : ', minval(MVX_all(MVX_all>-999.)), '/', maxval(MVX_all(MVX_all>-999.))     
 
      print*, 'Write in Q.dat'
      do IK=1, NK
        do ITH=1, NTH
          do JJ = 1,3
            do II = 1,3
              write(22,'(E20.12,X)',    advance='no') Q_all(II,JJ,ITH,IK) 
            end do
          end do
        end do
      end do
      ...
      print*, 'END'
      print*, '   => Min/Max : ', minval(Q_all(Q_all>-999.)), '/', maxval(Q_all(Q_all>-999.))
      
      print*, 'Write in LAMBDA1.dat'
      do IK=1, NK
        do ITH=1, NTH
          write(23,'(E20.12,X)',    advance='no') LAMBDA1(ITH,IK) 
        end do
      end do
      write(23, '(A)', advance="yes") ''
      print*, 'END'
      print*, '   => Min/Max : ', minval(LAMBDA1), '/', maxval(LAMBDA1)
      
      print*, 'Write in LAMBDA2.dat'
      do IK=1, NK
        do ITH=1, NTH
          write(24,'(E20.12,X)',    advance='no') LAMBDA2(ITH,IK) 
        end do
      end do
      write(24, '(A)', advance="yes") ''
      print*, 'END'
      print*, '   => Min/Max : ', minval(LAMBDA2), '/', maxval(LAMBDA2)
      
      print*, 'Write in SDS1.dat'
      do IK=1, NK
        do ITH=1, NTH
          write(25,'(E20.12,X)',    advance='no') SDS1(ITH,IK) 
        end do
      end do
      write(25, '(A)', advance="yes") ''
      print*, 'END'
      print*, '   => Min/Max : ', minval(SDS1), '/', maxval(SDS1)
      
      print*, 'Write in LM_DENSITY.dat'
      do IK=1, NK
        do ITH=1, NTH
          write(26,'(E20.12,X)',    advance='no') LM_DENSITY(ITH,IK) 
        end do
      end do
      write(26, '(A)', advance="yes") ''
      print*, 'END'
      print*, '   => Min/Max : ', minval(LM_DENSITY), '/', maxval(LM_DENSITY)
      
      print*, 'Write in Q_BK.dat'
      do IK=1, NK
        do ITH=1, NTH
          write(27,'(E20.12,X)',    advance='no') Q_BK_all(ITH,IK) 
        end do
      end do
      write(27, '(A)', advance="yes") ''
      print*, 'END'
      print*, '   => Min/Max : ', minval(Q_BK_all), '/', maxval(Q_BK_all)
      
      print*, 'Write in UC_BINS.dat'
      do IK=1, NK
        do II=1, SDSX_NUC
          write(28,'(E20.12,X)',    advance='no') UC_BINS_all(II,IK) 
        end do
      end do
      write(28, '(A)', advance="yes") ''
      print*, 'END'
      print*, '   => Min/Max : ', minval(UC_BINS_all), '/', maxval(UC_BINS_all)
      
      print*, 'Write in CP_BINS.dat'
      do IK=1, NK
        do JJ=1, SDSX_NCP
          write(29,'(E20.12,X)',    advance='no') CP_BINS_all(JJ,IK) 
        end do
      end do
      write(29, '(A)', advance="yes") ''
      print*, 'END'
      print*, '   => Min/Max : ', minval(CP_BINS_all), '/', maxval(CP_BINS_all)
      
      print*, 'Write in UC_CP_PDF.dat'
      do IK=1, NK
        do ITH=1, NTH
          do II=1, SDSX_NUC
            do JJ=1, SDSX_NCP
              write(30,'(E20.12,X)', advance='no') UC_CP_PDF_all(II,JJ,ITH,IK) 
            end do
          end do
        end do
      end do
      write(30, '(A)', advance="yes") ''
      print*, 'END'
      print*, '   => Min/Max : ', minval(UC_CP_PDF_all), '/', maxval(UC_CP_PDF_all)
...
!
      ...
      !!!!!!!!!!!!!!!!!!!!!! for Python comparison !!!!!!!!!!!!!!!!!!!!!
#ifdef W3_STXd
      open(8, file="NUC.dat",     status="new", action="write")
      write(8, '(I20)') SDSX_NUC
      close(8)
      
      open(9, file="NCP.dat",     status="new", action="write")
      write(9, '(I20)') SDSX_NCP
      close(9)
      
      open(11, file="WINS.dat",     status="new", action="write")
      do IK = 1, NK
        do IK2 = 1, NK
          write(11, '(E20.12,X)', advance="no") WINS(IK2,IK)
        end do
        write(11, '(A)', advance="yes") ''
      end do
      close(11)
      
      open(12, file="FCORFACT.dat", status="new", action="write")
      do IK = 1, NK
        write(12, '(E20.12,X)', advance="no") FCORFACT(IK)
      end do
      write(12, '(A)', advance="yes") ''
      close(12)
      
      open(13, file="IITH_COSTH_SINTH.dat", status="new", action="write")
      do IITH = IITH1_INT, IITH2_INT
        write(13, '(I10,X,E20.12,X,E20.12)', advance="yes") IITH, COSTH(IITH), SINTH(IITH)
      end do
      close(13)
      
      open(14, file="Q_FACT0.dat",     status="new", action="write")
      do II = 1, 3
        do JJ = 1, 3
          write(14, '(I1,X,I1,X)', advance="no") II, JJ
          do IK = 1, NK
            write(14, '(E20.12,X)', advance="no") Q_FACT0(IK,JJ,II)
          end do
          write(14, '(A)', advance="yes") ''
        end do
      end do
      ...
      open(14, file="Q_FACT2.dat", status="new", action="write")
      do II = 1, 3
        do JJ = 1, 3
          write(14, '(I1,X,I1,X)', advance="no") II, JJ
          do ITH = IITH1_INT, IITH2_INT
            write(14, '(E20.12,X)', advance="no") Q_FACT2(ITH,JJ,II)
          end do
          write(14, '(A)', advance="yes") ''
        end do
      end do
...
!/
      ...
      !!!!!!!!!!!!!!!!!!!!!! for Python comparison !!!!!!!!!!!!!!!!!!!!!
#ifdef W3_STXdd
      LOGICAL, SAVE :: CREATE_FILE  = .TRUE.
      INTEGER, SAVE :: Nout_PVC     = 0, &
                       Nout_PVC_max = 1000
#endif
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!/
...
#ifdef W3_STXdd
      if ( CREATE_FILE ) then
        open(20, file="PCV.dat",   status="new", action="write")
        CREATE_FILE = .FALSE.
      else
        open(20, file="PCV.dat",   status="old", position="append", action="write" )
      end if
      
      DO jj = 1, Ncp
        DO ii = 1, Nuc
         
          !if (Uc_Cp_pdf(ii,jj) .GT. 1.e-25) then
      
            if ( Nout_PVC .LT. Nout_PVC_max ) then
      
              Nout_PVC = Nout_PVC+1
      
              print*, 'Nout_PVC = ', Nout_PVC
      
              write(20,'(E20.12,X)',    advance='no') mvx
              write(20,'(3(E20.12,X))', advance='no') Q(:,1)
              write(20,'(3(E20.12,X))', advance='no') Q(:,2)
              write(20,'(3(E20.12,X))', advance='no') Q(:,3)
              write(20,'(E20.12,X)',    advance='no') Uc(ii,jj)
              write(20,'(E20.12,X)',    advance='no') Cp(ii,jj)
              write(20,'(E20.12,X)',    advance='no') xi(ii,jj)
              write(20,'(E20.12,X)',    advance='no') rho(ii,jj)
              write(20,'(E20.12,X)',    advance='no') alpha(ii,jj)
              write(20,'(E20.12,X)',    advance='no') phi(ii,jj)
              write(20,'(E20.12,X)',    advance='no') erf(0.5*SQRT(2.)*phi(ii,jj))
              write(20,'(E20.12,X)',    advance='no') Uc_Cp_pdf(ii,jj)
...
#ifdef W3_STXdd
      if ( CREATE_FILE ) then
        open(20, file="PCV_fast.dat",   status="new", action="write")
        CREATE_FILE = .FALSE.
      else
        open(20, file="PCV_fast.dat",   status="old", position="append", action="write" )
      end if
#endif
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        DO ii = 1, Nuc
          ...
          if ( Nout_PVC .LT. Nout_PVC_max ) then
            ...
            if (Uc_Cp_pdf(ii,jj) .GT. 1.e-25) then
              ...
              write(20,'(E20.12,X)',    advance='no') Uc(ii,jj)
              write(20,'(E20.12,X)',    advance='no') Cp(ii,jj)
              write(20,'(E20.12,X)',    advance='no') xi
              write(20,'(E20.12,X)',    advance='no') rho
              write(20,'(E20.12,X)',    advance='no') alpha
              write(20,'(E20.12,X)',    advance='no') phi
              write(20,'(E20.12,X)',    advance='no') erf(0.5*SQRT(2.)*phi)
              write(20,'(E20.12,X)',    advance='no') Uc_Cp_pdf(ii,jj)
              write(20,'(A)',           advance='yes') ''
